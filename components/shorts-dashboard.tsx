...
const regions = ["India", "Delhi NCR", "Mumbai", "Bengaluru", "Pune", "Hyderabad", "United States", "United Kingdom", "Australia"] as const;
...
type DrawerView = "why" | "dna" | "evolution" | "opportunities" | "build" | "hooks" | "distribution" | "evidence";
    ...
    <article className="group border-b border-[#ded9d1] bg-[#fbfaf7] transition-colors hover:bg-[#f8f4ed]">
      <button type="button" onClick={onSelect} className="grid w-full gap-4 p-4 text-left sm:grid-cols-[11rem_minmax(0,1fr)] sm:p-5 focus-visible:outline-none">
        <div className="relative aspect-video overflow-hidden bg-[#e4dfd5] sm:aspect-[16/10]">
          ...
          <img src={short.thumbnailUrl} alt="" onError={(event) => { event.currentTarget.style.display = "none"; }} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" loading="lazy" />
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[9px] font-semibold tracking-[0.13em] text-[#bc553d]">
            ...
            <span className="font-medium tracking-normal text-[#817a71]">{trend.metrics.freshness}</span>{trend.metrics.clusterSize ? <span className="font-medium tracking-normal text-[#817a71]">{trend.metrics.clusterSize} Shorts in pattern</span> : null}{trend.analysis.state && trend.analysis.state !== "unknown" ? <span className={`font-semibold tracking-[0.08em] ${trend.analysis.state === "cooling" || trend.analysis.state === "fading" ? "text-[#b34d42]" : trend.analysis.state === "viral" || trend.analysis.state === "accelerating" ? "text-[#3f8b5c]" : "text-[#817a71]"}`}>{trend.analysis.state.toUpperCase()}</span> : null}
          </div>
...
function OpportunityCard({ opportunity, selected, onSelect, onBuild }: { opportunity: PersonalizedOpportunity; selected: boolean; onSelect: () => void; onBuild?: () => void }) {
  return (
    <article className={`border-t border-[#e4dfd7] py-4 ${selected ? "bg-[#fffaf5]" : ""}`}>
      ...
      <div className="mt-3 flex items-center justify-between gap-3"><p className="text-[10px] leading-4 text-[#8a8177]">{opportunity.profileConnection ? `${opportunity.profileConnection} · ` : ""}{opportunity.drivers.slice(0, 3).join(" · ")}</p>{onBuild ? <button type="button" onClick={onBuild} className="shrink-0 border border-[#c9b2a7] px-2.5 py-1.5 text-[10px] font-semibold text-[#9b4634] hover:bg-[#f9ebe5]">Build this</button> : null}</div>
    </article>
  );
}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-12 sm:px-7"><div className="sr-only" aria-live="polite">{copiedLabel}</div>
          <section className="border-b border-[#ddd8cf] py-6" aria-labelledby={titleId}>
            <div className="relative aspect-[16/7] overflow-hidden bg-[#e4dfd5]">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={trend.representative.thumbnailUrl} alt="" onError={(event) => { event.currentTarget.style.display = "none"; }} className="h-full w-full object-cover" />
              <span className="absolute bottom-2 left-2 bg-[#242522]/90 px-2 py-1 text-[9px] font-semibold tracking-[0.12em] text-white">{trend.representative.mode === "live" ? "OBSERVED" : "DEMO"}</span>
            </div>
            <div className="mt-4 flex items-start justify-between gap-4"><div><p className="text-[9px] font-semibold tracking-[0.14em] text-[#bc553d]">{trend.category.toUpperCase()} · {trend.metrics.freshness.toUpperCase()}</p><h2 id={titleId} className="mt-2 text-2xl font-semibold leading-8 tracking-[-0.02em] text-[#24221f]">{trend.representative.title}</h2><p className="mt-1 text-[12px] text-[#777067]">{trend.representative.channel}</p></div><a href={trend.representative.url} target="_blank" rel="noreferrer" aria-label={`Open ${trend.representative.title} on YouTube`} className="mt-1 shrink-0 text-[#bc553d] hover:text-[#873d2d]"><ExternalLink className="size-4" aria-hidden="true" /></a></div>
            <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-[11px] text-[#625c54]"><span><strong className="font-semibold text-[#2d2a26]">{formatMetric(trend.representative.views)}</strong> views</span><span><strong className="font-semibold text-[#2d2a26]">{formatMetric(trend.representative.likes)}</strong> likes</span><span><strong className="font-semibold text-[#bc553d]">{score(trend.metrics.momentum)}</strong> momentum</span></div>
          </section>
          <nav className="sticky top-0 z-10 -mx-5 border-b border-[#ddd8cf] bg-[#fbfaf7]/95 px-5 py-3 backdrop-blur sm:-mx-7 sm:px-7" aria-label="Signal actions">
            <p className="text-[9px] font-semibold tracking-[0.15em] text-[#8a8177]">WHAT DO YOU WANT TO DO?</p>
            <div className="mt-2 grid grid-cols-2 gap-1.5 sm:grid-cols-3">
              ...
              <button type="button" onClick={() => setDrawerView("evolution")} className={`border px-2 py-2 text-left text-[10px] font-semibold ${drawerView === "evolution" ? "border-[#d6a896] bg-[#f9ebe5] text-[#9b4634]" : "border-[#e4dfd7] text-[#625c54] hover:border-[#c3a99b]"}`}>Show trend evolution</button>
          ...
          {drawerView === "evidence" ? <section className="border-b border-[#ddd8cf] py-7" aria-labelledby="focused-evidence-heading"><div className="flex items-center justify-between gap-3"><div><p className="text-[9px] font-semibold tracking-[0.14em] text-[#bc553d]">SOURCE EVIDENCE</p><h3 id="focused-evidence-heading" className="mt-1 text-2xl font-semibold">What we actually observed.</h3></div><span className="text-[9px] font-semibold tracking-[0.12em] text-[#8a8177]">{trend.representative.mode === "live" ? "LIVE" : "DEMO"}</span></div><EvidenceRow trend={trend} />{trend.relatedShorts.map((relatedTrend) => <EvidenceRow key={relatedTrend.id} trend={{ ...trend, representative: relatedTrend }} />)}</section> : null}
           {drawerView === "evolution" ? <section className="border-b border-[#ddd8cf] py-7" aria-labelledby="focused-evolution-heading"><div className="flex items-center justify-between gap-3"><div><p className="text-[9px] font-semibold tracking-[0.14em] text-[#bc553d]">TREND EVOLUTION</p><h3 id="focused-evolution-heading" className="mt-1 text-2xl font-semibold">What changed, and what to watch.</h3></div><TrendingUp className="size-4 text-[#bc553d]" aria-hidden="true" /></div><div className="mt-5 grid gap-4 border-y border-[#e4dfd7] py-4 sm:grid-cols-3"><div><p className="text-[9px] font-semibold tracking-[0.12em] text-[#8a8177]">STATE</p><p className="mt-1 text-sm font-semibold">{analysis.state ? capitalize(analysis.state) : "Unavailable"}</p></div><div><p className="text-[9px] font-semibold tracking-[0.12em] text-[#8a8177]">MOMENTUM</p><p className="mt-1 text-sm font-semibold text-[#bc553d]">{score(trend.metrics.momentum)}</p></div><div><p className="text-[9px] font-semibold tracking-[0.12em] text-[#8a8177]">BASIS</p><p className="mt-1 text-[11px] leading-5 text-[#766f66]">{trend.metrics.momentumBasis === "cross-sectional" ? "Current result-set estimate" : capitalize(trend.metrics.momentumBasis)}</p></div></div><div className="mt-5 space-y-4 border-l-2 border-[#d6a896] pl-4 text-[13px] leading-6 text-[#4d4942]"><p><strong className="font-semibold text-[#2d2a26]">Stage:</strong> {analysis.evolution.stage}</p><p><strong className="font-semibold text-[#2d2a26]">Movement:</strong> {analysis.evolution.movement}</p><p><strong className="font-semibold text-[#2d2a26]">Next watch:</strong> {analysis.evolution.nextWatch}</p></div><p className="mt-5 text-[10px] leading-5 text-[#8a8177]">Momentum is a cross-sectional estimate until historical observations are available.</p></section> : null}
...
export function ShortsDashboard() {
  ...
  function updateFilter(key: keyof ScanFilters, value: string) {
    setAiSubniches([]);
    const nextFilters = ((current: ScanFilters): ScanFilters => {
      if (key === "category") {
    ...
    if (samplePreview) setSnapshot(getSampleSnapshot(nextFilters));
  }

  async function suggestSubniches() {
    setTaxonomyMessage("Refining this branch in the background…");
    try {
      const response = await fetch("/api/taxonomy", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ region: filters.region, category: filters.category, subcategory: selectedSubcategory, query: filters.subNiche ?? "" }) });
  ...
  async function scan() {
    ...
    const query = new URLSearchParams({ region: filters.region, category: filters.category, query: filters.query ?? "", language: filters.language ?? "", subcategory: advancedFiltersOpen ? selectedSubcategory : "", subNiche: advancedFiltersOpen ? filters.subNiche ?? "" : "", niche: advancedFiltersOpen ? filters.niche : "", platform: filters.platform, timeRange: filters.timeRange });
    try {
  ...
  const topSignal = displayedTrends[0];
  const fastestTrend = [...(snapshot?.trends ?? [])].sort((a, b) => (b.metrics.momentum ?? -1) - (a.metrics.momentum ?? -1))[0];
  const emergingTrend = [...(snapshot?.trends ?? [])].filter((trend) => trend.metrics.saturation !== "crowded").sort((a, b) => (b.metrics.opportunityScore ?? b.metrics.momentum ?? -1) - (a.metrics.opportunityScore ?? a.metrics.momentum ?? -1))[0];
  const snapshotTrends = [topSignal, fastestTrend, emergingTrend].filter((trend): trend is Trend => Boolean(trend));
  const topFormat = topSignal?.analysis.mode === "sample" ? topSignal.analysis.viralDna.format : "Open the signal to reveal its format.";
  const topHook = topSignal?.analysis.mode === "sample" ? topSignal.analysis.viralDna.hook : "Open the signal to reveal its hook.";
  const topOpportunity = emergingTrend?.opportunities[0]?.title ?? "Open a signal to find your angle.";
  const trendOfMoment = topSignal?.analysis.mode === "sample" ? topSignal.analysis.evolution.movement : "Open a signal for an evidence-grounded read of what changed.";
  ...
  return (
    <main className="min-h-screen bg-[#f4f1eb] text-[#24221f]">
      ...
      <div className="mx-auto max-w-[1380px] px-4 pb-16 sm:px-7 lg:px-10">
         <section className="border-b border-[#d8d2c9] pb-6 pt-8 sm:pt-10" aria-labelledby="page-title">
           <div className="max-w-3xl"><p className="text-[10px] font-semibold tracking-[0.17em] text-[#bc553d]">THE INTELLIGENCE LAYER FOR THE TECH INTERNET</p><h1 id="page-title" className="mt-3 text-3xl font-semibold leading-[1.08] tracking-[-0.03em] text-[#24221f] sm:text-4xl">Three things worth your attention.</h1><p className="mt-3 max-w-2xl text-[14px] leading-6 text-[#6f685f]">MOMENTUM turns what is changing into a clear next move: understand it, find your angle, and make something useful.</p></div>
           <label className="mt-6 flex max-w-3xl items-center gap-3 border border-[#cfc7bc] bg-[#fbfaf7] px-4 py-3 focus-within:border-[#bc553d]"><Search className="size-4 shrink-0 text-[#bc553d]" aria-hidden="true" /><span className="sr-only">What are you looking for?</span><input value={filters.query ?? ""} onChange={(event) => updateFilter("query", event.target.value)} placeholder="Delhi street food, fitness trends in India, budget travel Japan" className="min-w-0 flex-1 bg-transparent text-[13px] text-[#272521] outline-none placeholder:text-[#9a938a]" /><span className="hidden text-[9px] font-semibold tracking-[0.1em] text-[#9a938a] sm:inline">NATURAL LANGUAGE</span></label>
          <div className="mt-8 flex flex-col gap-2 sm:flex-row sm:items-stretch">
             <div className="hidden flex-1 items-stretch border border-[#cfc7bc] bg-[#fbfaf7] lg:flex">
               ...
               <details className="group relative border-l border-[#ddd8cf]" onToggle={(event) => setAdvancedFiltersOpen(event.currentTarget.open)}>
                 <summary className="flex h-full cursor-pointer list-none items-center gap-2 px-3 text-[10px] font-semibold text-[#6f685f] marker:hidden hover:text-[#bc553d]"><span>Refine</span><ChevronDown className="size-3.5 transition-transform group-open:rotate-180" aria-hidden="true" /></summary>
                 <div className="absolute right-0 top-[calc(100%+8px)] z-30 grid w-[min(34rem,calc(100vw-2rem))] gap-3 border border-[#d8d2c9] bg-[#fbfaf7] p-4 shadow-2xl sm:grid-cols-2"><FilterSelect label="SUBCATEGORY" value={selectedSubcategory} options={stringOptions(subcategoryOptions)} onChange={(value) => updateFilter("subcategory", value)} /><FilterSelect label="SUB-NICHE" value={filters.subNiche ?? ""} options={stringOptions(subnicheOptions)} onChange={(value) => updateFilter("subNiche", value)} /><button type="button" onClick={suggestSubniches} className="text-left text-[10px] font-semibold text-[#bc553d] hover:text-[#873d2d]">Refine sub-niche with AI</button></div>
               </details>
             </div>
